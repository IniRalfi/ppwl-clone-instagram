...
import { ArrowLeft, Check, CheckCheck, Loader2, MessageCircle, Search, Send, X, AlertTriangle } from "lucide-react";
import { MessagesPageSkeleton } from "../components/ui/PageState";
import { toast } from "sonner";
import { apiClient } from "../services/api.client";
import { useAuthStore } from "../store/auth.store";
import { useMessageStore } from "../store/message.store";
import { useDirectMessagesRealtime, type RealtimeMessagePayload } from "../hooks/useDirectMessagesRealtime";
...
function getAvatar(user?: ChatUser | null) {
  ...
  return user.avatarUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(user.name || user.username)}`;
}
...
export default function MessagesPage() {
  ...
  const markRoomAsRead = useCallback(async (roomId: string) => {
    try {
      const res = await apiClient.post<{ data: { updatedCount: number } }>(`/messages/${roomId}/read`, {});
  ...
  const openUser = async (targetUser: ChatUser) => {
    ...
    try {
      const res = await apiClient.get<{
        data: { room: { id: string; updatedAt: string } | null; otherUser: ChatUser; messages: ChatMessage[] };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setDraft("");
      setMessages((prev) => (prev.some((item) => item.id === res.data.id) ? prev : [...prev, res.data]));
      if (res.data.roomId) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        const res = await apiClient.get<{ data: ChatUser[] }>(`/users?search=${encodeURIComponent(query)}`);
  ...
  const handleRealtimeRead = useCallback((payload: { roomId: string }) => {
    if (payload.roomId === selectedRoomId) {
  ...
  useDirectMessagesRealtime(user?.id, token, handleRealtimeMessage, handleRealtimeRead);
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-49px)] w-full max-w-6xl overflow-hidden bg-ig-background text-ig-text md:h-screen md:border-x md:border-ig-border">
      {/* Room List */}
      <aside className={`${showConversation ? "hidden md:flex" : "flex"} w-full flex-col border-r border-ig-border md:w-[360px] md:min-w-[360px]`}>
        <div className="border-b border-ig-border px-4 py-4">
          ...
          <div className="relative mt-3">
            ...
            />
            {search && (
              <button type="button" onClick={() => setSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-ig-secondary-text hover:text-ig-text">
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {search.trim().length >= 2 ? (
            <UserList users={searchResults} onSelect={openUser} emptyText="Tidak ada user yang cocok." />
          ) : isRoomsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
              ...
              <button onClick={loadRooms} className="mt-3 text-sm font-semibold text-ig-primary hover:opacity-80">
                Coba Lagi
              </button>
            </div>
          ) : rooms.length === 0 ? (
            ...
            rooms.map((room) => (
              ...
              >
                <div className="relative flex-shrink-0">
                  <img src={getAvatar(room.otherUser)} alt={room.otherUser.username} className="h-14 w-14 rounded-full object-cover" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between">
                    <p className={`truncate text-sm ${room.unreadCount > 0 ? "font-bold" : "font-semibold"}`}>
                      {room.otherUser.name}
                    </p>
                    {room.lastMessage && (
      ...
      {/* Chat Area */}
      <section className={`${showConversation ? "flex" : "hidden md:flex"} min-w-0 flex-1 flex-col`}>
        {selectedUser ? (
          <>
            {/* Chat Header */}
            <div className="flex items-center gap-3 border-b border-ig-border px-4 py-3">
              ...
              <Link to={`/profile/${selectedUser.username}`} className="flex items-center gap-3 min-w-0">
                <img src={getAvatar(selectedUser)} alt={selectedUser.username} className="h-10 w-10 rounded-full object-cover" />
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold">{selectedUser.name}</p>
                  <p className="truncate text-xs text-ig-secondary-text">@{selectedUser.username}</p>
                </div>
              </Link>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4">
              {isMessagesLoading ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-center text-ig-secondary-text">
                  <img src={getAvatar(selectedUser)} alt={selectedUser.username} className="mb-4 h-20 w-20 rounded-full object-cover" />
                  <p className="text-base font-semibold text-ig-text">@{selectedUser.username}</p>
                  <p className="mt-1 max-w-xs text-sm">Mulai percakapan. Pesan pertama kamu akan muncul di sini.</p>
                </div>
              ) : (
                        ...
                        <div className={`flex max-w-[75%] flex-col ${isMine ? "items-end" : "items-start"}`}>
                          ...
                          {isMine && (
                            <div className="mt-0.5 flex items-center gap-1 px-1">
                              <span className="text-[10px] text-ig-secondary-text">{formatTime(message.createdAt)}</span>
                              {message.isRead ? (
          ...
          <div className="flex h-full flex-col items-center justify-center px-6 text-center text-ig-secondary-text">
            ...
            <p className="mt-2 max-w-sm text-sm">Pilih percakapan atau cari user untuk mulai ngobrol realtime.</p>
          </div>
        )}
      </section>
    </div>
...
function UserList({ users, onSelect, emptyText }: { users: ChatUser[]; onSelect: (user: ChatUser) => void; emptyText: string }) {
  if (users.length === 0) {
    ...
    <div className="py-2">
      {users.map((user) => (
        <button
          ...
          className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-ig-elevated-bg"
        >
          <img src={getAvatar(user)} alt={user.username} className="h-11 w-11 rounded-full object-cover" />
          <div className="min-w-0">
