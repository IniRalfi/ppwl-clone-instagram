...
import { Heart, MessageCircle, Send, Bookmark, MoreHorizontal, ChevronLeft, ChevronRight, User, UserPlus, Check } from 'lucide-react';
...
export const PostCard: React.FC<PostCardProps> = ({
  id, authorId, username, avatarUrl, imageUrls, caption, likesCount, commentsCount = 0, timeAgo,
  ...
  const handleLikeToggle = async () => {
    if (!currentUserId) { toast.error("Login dulu untuk memberi like!"); return; }
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    isDown.current = true; setIsDragging(true);
  ...
  const handleMouseLeaveOrUp = () => {
    ...
    isDown.current = false; setIsDragging(false);
    scrollToImage(Math.round(scrollContainerRef.current.scrollLeft / scrollContainerRef.current.clientWidth));
  };

  const handleMouseMove = (e: React.MouseEvent) => {
    if (!isDown.current || !scrollContainerRef.current) return;
    e.preventDefault();
    scrollContainerRef.current.scrollLeft = scrollLeft.current - (e.pageX - scrollContainerRef.current.offsetLeft - startX.current) * 1.2;
  };
  ...
  const renderHoverCard = (isFooter = false) => (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 text-center border-t border-b border-ig-border py-3 mb-4">
        <div>
          <div className="font-bold text-sm text-ig-text">
            {hoverStats ? hoverStats.postsCount : postsCount}
          </div>
          <div className="text-[11px] text-ig-secondary-text">posts</div>
        </div>
        <div>
          <div className="font-bold text-sm text-ig-text">
            {hoverStats ? hoverStats.followers : followers}
          </div>
          <div className="text-[11px] text-ig-secondary-text">followers</div>
        </div>
        <div>
          <div className="font-bold text-sm text-ig-text">
            {hoverStats ? hoverStats.following : following}
          </div>
      ...
      {currentUserId && currentUserId !== authorId && (
        <button
          ...
          disabled={isFollowLoading || isHoverFollowed}
        ...
        {tags.length > 0 && (
          <button
            className={`absolute bottom-3 left-3 p-2 rounded-full transition-all duration-200 z-20 border-none cursor-pointer flex items-center justify-center backdrop-blur-sm ${
              showTags ? 'bg-blue-500 text-white scale-105' : 'bg-black/60 text-white/90 hover:bg-black/80 hover:scale-105'
            }`}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-1.5">
              <Button variant="ghost" size="icon" onClick={handleLikeToggle} disabled={isLikeLoading}
