...
import { postRateLimit, readRateLimit } from "@/middleware/rate-limit.middleware";
import {
  getPostsSchema,
  getPostByIdSchema,
  createPostSchema,
...
export const postRoutes = new Elysia({ prefix: "/posts" })
  ...
  // 🛡️ Rate limit: 100 requests per menit (read operation)
  .get(
    "/",
    async ({ query, getCurrentUser }) => {
      ...
      const take =
        Number.isInteger(requestedLimit) && requestedLimit > 0 ? Math.min(requestedLimit, 50) : 10;
  ...
  .get(
    "/:id",
    async ({ params: { id }, getCurrentUser, set }) => {
      const user = await getCurrentUser();
      const currentUserId = user?.id;
  ...
  .post(
    ...
    async ({ body, requireUser, set }) => {
      try {
        ...
        set.status =
          error.message?.includes("Format") || error.message?.includes("Ukuran") ? 400 : 500;
  ...
  .delete(
    "/:id",
    async ({ params: { id }, requireUser, set }) => {
      try {
        const user = await requireUser();
        if (!user) return;
